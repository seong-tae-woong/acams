...
## ⚠️ 먼저 정할 결정 3개 → 현재 **1개(임베딩 벤더)만 열림**

> 갱신 2026-07-03: **결정 2(비동기 인프라)·3(검수 흐름)은 아래대로 해소** — 2는 방식 A 채택, 3은 P1 검수 패턴 계승. 남은 **결정 1(임베딩 벤더)**도 RAG가 켜지는 **P2b-2에서만** 필요 → **P2b-1(수집 최소 루프)은 결정 없이 바로 착수 가능.**
...
### 결정 2 — 비동기 잡 인프라 → ✅ **방식 A 채택**(2026-07-03)
Fluid Compute로 함수 상한이 **300초**(Hobby)·**800초**(Pro)로 올라 60초 벽은 해소. 그래도 다페이지 스캔은 단건 300초도 위험 → **페이지별로 쪼개는 방식 A가 정답**(진행률·페이지별 재시도·플랜 무관). 옵션:
| 방식 | 내용 | 적합 |
|---|---|---|
| **A. 페이지/문항별 순차(클라 오케스트레이션)** ✅ | 모의고사 섹션처럼 클라가 페이지마다 짧은 호출 순차 | **P2b-1 채택**(신규 인프라 0, 모의고사 패턴 재사용) |
| B. 큐 테이블 + Vercel Cron 폴링 | `SourceDocument.status` 큐, Cron이 처리 | 대량·무인(P2b-3), Cron 잦은 주기는 Pro |
| C. 외부 큐/잡 (Inngest·QStash·Trigger.dev) | 진짜 백그라운드 워커 | 대량·안정, 벤더 추가(P2b-3) |
- **결정**: **A 채택**(모의고사 섹션 순차 재사용). ⚠️ 함수 body **4.5MB 상한** → 파일은 클라에서 Blob로 직접 업로드(함수 경유 X). 대량 무인은 P2b-3에서 B/C, Batch API(−50%)는 대량 재처리.

### 결정 3 — 검수 흐름(업로드 문항) → ✅ **P1 패턴 계승**(2026-07-03)
업로드 파싱은 정답키·구조 오류 위험이 생성보다 큼. 수집 게이트 = **글자깨짐·구조결함**(기존 `review.ts` 재사용/확장) → 강사 검수 필수. 자동+사람 2단은 P1과 동일 철학 → **P1 흐름 그대로 계승**으로 확정. CLOVA OCR 교차검증은 선택(한국어 충실도, P2b-3에서 검토).
...
2. [x] 결정 확정 — 비동기=방식 A · 검수=P1 계승(2026-07-03). 남은 **임베딩 벤더**는 P2b-2 착수 시 선택
