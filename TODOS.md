...
## 소통·알림 (2026-06-17 /plan-ceo-review에서 도출)

### 22. 미납 알림 SMS 발송 (학부모 실제 도달) (P2)
- **무엇**: 청구/수납/미납 → 미납 관리의 '미납 알림 일괄/개별 발송'을 학부모 **휴대폰 SMS**로도 발송. 현재는 앱 내 알림 + 웹푸시만.
- **왜**: 웹푸시는 학부모가 PWA 설치 + 알림 허용한 경우에만 도착 → 실제로는 대부분 미도달(특히 iOS는 홈화면 추가 필요). 미납 독촉은 받는 사람이 봐야 의미가 있는데, 지금은 거의 안 보는 통로로만 나감. SMS는 거의 전원 도달.
- **현재 상태**: SMS 인프라 `sendSms(receiver, msg)` 이미 동작 (`src/lib/sms/solapi.ts`, HMAC-SHA256, 90바이트 초과 시 자동 LMS). 단, 지금은 **계정·비밀번호 안내에만** 사용(`api/students/route.ts`, `api/*/reset-password`). 미납 발송 경로는 `OverdueTab.tsx`의 `sendBatchOverdueNotifications`/`sendOverdueNotification` → `POST /api/communication/notifications` → `sendPushToStudents`(웹푸시)뿐. false-success 집계 버그는 2026-06-17 수정 완료(별개).
- **출발점**:
  - 알림 POST 라우트(또는 전용 엔드포인트)에서 수신 학생 → **학부모 연락처** 조회(`reset-password` 라우트의 `parentPhone` 조회 패턴 재사용) → `sendSms` 호출.
  - SMS 본문은 `generateOverdueContent`(`_shared.ts`)를 SMS용으로 축약 — `[결제하기] 버튼을 눌러` 문구는 SMS에서 무의미하므로 `/mobile/payments` 링크로 대체.
  - 발송 전 **확인 다이얼로그**(대상 N명·건당 비용 인지) + 학원 SMS 설정 on/off 가드 + 결과 집계(발송/연락처없음/실패)를 정직하게 표시.
  - 수신 대상은 결제 주체인 **학부모**(학생 본인 아님).
- **확장 옵션 (별도, Approach C)**: 카카오 **알림톡**(저렴·리치, 단 템플릿 사전승인 외부 리드타임 필요) · 발송상태 DB 기록 · `Bill.notifiedAt` 기반 '최근 발송 제외' · 자동 미납 독촉 → 기존 `auto_notification` 시간기반 트리거 기획과 통합 검토.
- **선행 조건**: 학원별 SMS 비용·발신번호(`SOLAPI_SENDER`) 운영 정책 확인. 알림톡까지 가면 카카오 채널 + 템플릿 승인 선행.
- **Effort**: M (human ~반나절 / CC ~30~45분). 알림톡·자동화 포함 시 L.

## 테스트 (2026-06-20 /plan-eng-review에서 도출)

### 23. 컴포넌트 테스트(RTL/jsdom) 도입 (P2)
- **무엇**: vitest + @testing-library/react + jsdom 렌더 테스트 인프라 추가. 실제 컴포넌트를 가짜 DOM에 그려 텍스트·클릭·상태를 검증.
- **왜**: UI 비중 큰 Next 앱인데 컴포넌트 렌더 테스트 0개(현재 66개 전부 순수 함수). "학부모 화면 백지 죽음"(레거시 데이터 crash), "밴드·문장 모순 발행"(클릭→상태→재렌더 버그) 같은 사고를 자동으로 잡을 장치가 없음. #7(API integration 테스트)과 별개 — 그건 서버, 이건 화면.
- **현재 상태**: vitest 4.1.7 설치됨(#7 일부 진행), 단 RTL/jsdom 없음, `.test.tsx` 0개. 레벨테스트 리포트 개정(2026-06-20)의 레거시 렌더 회귀는 순수 술어(`showPlacement` 등) 추출로 우회 커버(plan-eng-review 3B).
- **출발점**: `npm i -D @testing-library/react @testing-library/jest-dom jsdom` → `vitest.config.ts`에 `environment: 'jsdom'` → 고가치 view부터(모바일 리포트 `mobile/reports/[id]`, 결제 `mobile/payments`). jsdom은 픽셀/실제 CSS는 못 봄(browse/Playwright 영역).
- **선행 조건**: 없음(독립 결정). 도입 시점은 별도 판단.
- **Effort**: S (셋업 human ~30분 / CC ~10min), 이후 view당 테스트 작성 비용.

## 인증 (2026-06-26 /plan-eng-review에서 도출)

### 24. 비밀번호 변경 코드 중복 정리 (모달 + 엔드포인트) (P3)
- **무엇**: 셀프 비밀번호 변경 UI·API의 중복을 공유 자원으로 통합. ① 모달 폼: super_admin 헤더와 GNB가 거의 동일한 모달을 각자 보유(2벌) → `PasswordChangeModal(open, onClose, endpoint)` 공유 컴포넌트로 추출. ② 엔드포인트: `api/auth/change-password`와 `api/super-admin/profile/password`가 ~95% 동일(현재비번·복잡도·history-3·token++·audit) → 역할무관 `auth/change-password` 하나로 통합하고 super_admin도 그걸 호출.
- **왜**: 2026-06-26 원장·강사 셀프 비번 변경 추가 시 office-hours/eng-review에서 '최소 churn' 우선으로 모달을 복사(A1)함. DRY 부채를 의식적으로 남긴 것 → 추후 정리. 보안 폼/로직은 단일 출처가 안전(한 곳만 고치면 됨).
- **현재 상태**: 모달 = `super-admin/SuperAdminHeader.tsx` + `components/admin/GNB.tsx` 2벌(폼·handleSubmit 거의 동일, endpoint만 다름). 엔드포인트 = `api/auth/change-password/route.ts`(역할무관, 원장·강사·super_admin 모두 동작) + `api/super-admin/profile/password/route.ts`(super_admin 전용, 기능 동일).
- **출발점**: `components/shared/PasswordChangeModal.tsx` 신설(폼+제출+toast, endpoint prop) → GNB·SuperAdminHeader가 트리거 버튼만 보유하고 모달 공용. 이후 super_admin UI를 `auth/change-password`로 전환 → `super-admin/profile/password` 제거(또는 deprecated 표기).
- **선행 조건**: 없음(독립). super_admin 비번 변경 회귀 QA 필요.
- **Effort**: S (human ~2시간 / CC ~20분)
