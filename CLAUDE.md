...
- 메시지 위계: 1차=미션+격차(대형학원 vs 동네학원, teal·navy), 2차=인강 수익(첫 무기·구매 트리거, 보라). 기능 7종은 "대형학원이 쓰던 무기" 톤. 섹션 순서: 히어로(미션)→격차→인강 수익→기능(데이터 통합 운영 포함)→모바일→창업자 노트→상담 CTA.
- 제약: "영국책방" 등 특정 학원명·실데이터 금지(더미만), **'AI' 과장 금지**(→"데이터 통합 운영"), 인강 수익은 산식형 예시+면책 캡션 필수, 브랜드명 "학원로그", 1차 CTA=상담폼(ConsultForm, `#consult`).
