...
### 완료 (Phase F) — 인강 메뉴 UI 프로토타입 (mock-only)

> 기획서(`AcaMS_기획서_v1_4.md`) + `화면 샘플/`의 인강 HTML 시안 기준으로 전체 UI 구현. **현 시점 데이터/로직은 모두 컴포넌트 내부 inline mock**, API·DB 모델은 추후 설계.

**라우트 (모두 `director/teacher` 권한)**

| 경로 | 페이지 |
|------|--------|
| `/ingang/lectures` | 강의 목록 (KPI 4종 + 필터 + 카드 그리드) |
| `/ingang/lectures/new` | 강의 등록 (영상 업로드 + 분류·태그 + 미리보기) |
| `/ingang/lectures/tags` | 강의 분류/태그 관리 |
| `/ingang/lectures/targets?tab=` | 수강 대상 지정 / 이수 조건 설정 / 재응시 관리 (3탭) |
| `/ingang/exams` | 시험 출제 에디터 (강의별 문제 CRUD, 객관식 정답 마킹, 배점) |
| `/ingang/completion?tab=` | 시청 현황 / 시험 응시 현황 (2탭, 점수 분포 차트 포함) |
| `/ingang/completion/stats` | 이수율 통계 (학생별 진도·점수·이수상태 표) |
| `/ingang/completion/notifications?tab=` | 미이수 알림 발송 (체크박스 + 알림톡 템플릿) / 이수증 발급 (실시간 미리보기) |

**구조**
- 레이아웃: `src/app/ingang/layout.tsx` — GNB + InGangSidebar + main + ToastContainer
- 사이드바: `src/components/ingang/InGangSidebar.tsx` — 3그룹(강의·시험·이수) 11개 항목, 보라색 테마(`#1e1b2e`/`#a78bfa`)
- 동일 경로 + 다른 탭 처리: URL search param(`?tab=cond|retry|exam|cert`) → `router.replace()`로 동기화 → 사이드바 active state도 같은 param에서 파생
- `useSearchParams()` 사용 페이지는 모두 `<Suspense>` 래핑 (Next.js 16 SSR 요구사항)
- GNB 진입: `src/components/admin/GNB.tsx`의 "인강" 링크 → `/ingang/lectures`

**미구현 (다음 단계)**
- DB 모델 설계 (Lecture, LectureTag, LectureTarget, ExamQuestion, ExamSubmission, ViewProgress, CompletionRecord 등)
- API 라우트 (`/api/ingang/*`) 및 store 추가
- 영상 업로드/스트리밍 (S3 + HLS, DRM 검토)
...
- [ ] **인강 백엔드**: 영상 업로드/스트리밍, 시청 진도 추적, 시험 응시·자동 채점, 이수증 PDF
