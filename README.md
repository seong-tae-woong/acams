...
│   │   │   ├── analytics/   # 통계 분석
│   │   │   └── settings/    # 계정 관리
│   │   ├── ingang/          # 인강 (강의 목록·등록·태그·수강대상 DB 연동, Cloudflare Stream 업로드)
│   │   │   ├── lectures/    # 강의 목록·등록·태그·수강대상
│   │   │   ├── exams/       # 시험 출제 에디터
│   │   │   └── completion/  # 시청·시험 응시 현황·이수율 통계·알림 발송·이수증
│   │   ├── mobile/          # 학부모·학생 PWA
...
Lecture             강의 (subjects/levels/targetGrades String[], cfVideoId, status DRAFT/PUBLISHED)
LectureTarget       수강 대상 지정 (Lecture ↔ Class 매핑)
AcademyTag          학원별 커스텀 태그 (tagType: subject|level|grade, @@unique[academyId,tagType,label])
```
...
| POST | /api/lectures | 강의 등록 (title, subjects, levels, targetGrades, cfVideoId, status) |
| GET | /api/lectures/[id] | 강의 상세 |
| PATCH | /api/lectures/[id] | 강의 수정 |
| DELETE | /api/lectures/[id] | 강의 삭제 |
| GET | /api/lectures/tags | 학원별 커스텀 태그 목록 |
| POST | /api/lectures/tags | 커스텀 태그 추가 (P2002 → 409) |
...
### 완료 (Phase F) — 인강 강의 관리 DB 연동 + Cloudflare Stream 업로드

**DB 모델**: `Lecture`, `LectureTarget`, `AcademyTag` (schema.prisma 추가)

**API 라우트** (`/api/lectures/*` — x-academy-id 필수)
- CRUD: `lectures/`, `lectures/[id]/`
- 커스텀 태그: `lectures/tags/`, `lectures/tags/[id]/`
- 영상 업로드 URL: `lectures/upload-url/` → Cloudflare Stream Direct Creator Upload
...
강의 등록 폼 → POST /api/lectures/upload-url → { uploadURL, uid }
  ...
  → 재생: https://iframe.videodelivery.net/{cfVideoId}
```

> **주의**: Cloudflare Stream 무료 tier는 0분 (업로드 불가). $5/월 1,000분 플랜 구매 필요.  
> 환경변수: `CF_ACCOUNT_ID`, `CF_STREAM_API_TOKEN` (Vercel 대시보드에도 등록 필요)

**라우트 (모두 `director/teacher` 권한)**

| 경로 | 페이지 | 데이터 |
|------|--------|--------|
| `/ingang/lectures` | 강의 목록 (카드 그리드) | DB 연동 |
| `/ingang/lectures/new` | 강의 등록 (영상 업로드 + 분류·태그 + 미리보기) | DB 연동 |
| `/ingang/lectures/tags` | 강의 분류/태그 관리 (학원별 커스텀 태그 추가/삭제) | DB 연동 |
| `/ingang/lectures/targets?tab=` | 수강 대상 지정 / 이수 조건 설정 / 재응시 관리 | 강의 목록 DB, 나머지 mock |
| `/ingang/exams` | 시험 출제 에디터 | mock |
| `/ingang/completion?tab=` | 시청 현황 / 시험 응시 현황 | mock |
| `/ingang/completion/stats` | 이수율 통계 | mock |
| `/ingang/completion/notifications?tab=` | 미이수 알림 발송 / 이수증 발급 | mock |

**미구현 (다음 단계)**
- 시청 진도 추적, 시험 응시·자동 채점 (ExamQuestion, ViewProgress, CompletionRecord 모델 필요)
- 알림톡 발송 연동 (Phase G와 통합)
- 이수증 PDF 발급
...
- [ ] **인강 백엔드**: 시청 진도 추적, 시험 응시·자동 채점, 이수증 PDF (영상 업로드는 Phase F에서 Cloudflare Stream 연동 완료)
