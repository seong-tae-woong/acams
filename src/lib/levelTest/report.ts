...
import type { SectionScore, LevelTestReportData, LevelTestReportSection, LevelTestPlacement, LevelTestPercentile } from './types';
...
/**
 * 동급생 분포에서 상위 % — 작을수록 상위. dist는 자기 점수 포함(N=dist.length).
 * 상위% = round((자기보다 높은 인원 + 1) / N * 100), [1,100] clamp. 동점은 같은 순위.
 */
export function computeTopPercent(score: number, dist: number[]): number {
  const n = dist.length;
  if (n === 0) return 100;
  const higher = dist.filter((v) => v > score).length;
  return Math.min(100, Math.max(1, Math.round(((higher + 1) / n) * 100)));
}
...
export function buildLevelTestReportData(input: {
  ...
  /** 동급생 총점 분포 (자기 포함). 백분위 계산용. N≥임계 + showAverage일 때만 사용. */
  cohortTotals?: number[];
  /** 동급생 영역 점수 분포 (자기 포함, sectionScores 배열). 영역별 백분위 계산용. */
  cohortSections?: SectionScore[][];
}): LevelTestReportData {
  ...
  // 동급생 백분위 — N≥임계 + showAverage일 때만(민감 상담이면 토글 끔). 발행 시 스냅샷.
  let percentile: LevelTestPercentile | null = null;
  const totals = input.cohortTotals ?? [];
  if (showAverage && totals.length >= LEVELTEST_COHORT_THRESHOLD) {
    const keyDist = new Map<string, number[]>();
    for (const member of input.cohortSections ?? []) {
      for (const sec of member) {
        const arr = keyDist.get(sec.key) ?? [];
        arr.push(sec.score);
        keyDist.set(sec.key, arr);
      }
    }
    percentile = {
      cohortSize: totals.length,
      total: computeTopPercent(input.totalScore, totals),
      sections: sectionScores.map((s) => ({ name: s.name, top: computeTopPercent(s.score, keyDist.get(s.key) ?? []) })),
    };
  }
