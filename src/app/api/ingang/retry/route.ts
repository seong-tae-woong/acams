...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error('[GET /api/ingang/retry]', err instanceof Error ? err.message : String(err));
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error('[POST /api/ingang/retry]', err instanceof Error ? err.message : String(err));
    return NextResponse.json({ error: '서버 오류가 발생했습니다.' }, { status: 500 });
  }
}
