...
const PALETTE_COLORS: Record<CurriculumPalette, string[]> = {
...
const UNIT_LABELS: Record<CurriculumUnitType, { label: string; suffix: string }> = {
...
───────────────────────────────────────────────── */
function ClassCard({ cls, showFee, onClick }: { cls: ClassItem; showFee: boolean; onClick?: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
          ...
          <section style={{ marginBottom: 52 }}>
            ...
            <div style={{
              ...
              {profile.classes.map((cls) => (
                <ClassCard key={cls.id} cls={cls} showFee={profile.showFees} onClick={() => setClassDetailId(cls.id)} />
              ))}
            </div>
          </section>
        )}
