      ...
      <MobileContentLoader loading={loading}>
        ...
        {report && (
          <div className="px-4 py-4 space-y-3">
            ...
            {/* 레벨 테스트 (객관 점수형) */}
            {report.kind === 'LEVEL_TEST' && (() => {
              const lt = report.data as unknown as LevelTestReportData;
              const clamp = (v: number) => Math.min(100, Math.max(0, v));
              return (
                <>
                  <div className="bg-white rounded-[12px] border border-[#e2e8f0] p-4">
                    <div className="flex items-baseline gap-2">
                      <span className="text-[40px] font-bold text-[#111827] leading-none tabular-nums">{lt.totalScore}</span>
                      <span className="text-[14px] text-[#6b7280]">/ 100점</span>
                      {lt.totalAverage != null && (
                        <span className="ml-auto text-[12px] font-medium text-[#374151] bg-[#f4f6f8] px-2.5 py-1.5 rounded-[8px] tabular-nums">
                          {lt.averageLabel} {lt.totalAverage}점
                        </span>
                      )}
                    </div>
                  </div>
